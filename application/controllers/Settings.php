...
class Settings extends CI_Controller
{
	...
	{
		$website_setting = $this->settings_model->getall_settings();
		$data = $website_setting["Prices"];
		$data["driverShare"] = $website_setting["driverShare"]["percent"];
	...
	public function updateSettings()
	{
		...
		foreach ($_POST as $key => $val) {
			if (strpos($key, "iRide_") === 0) {
				$name = substr($key, 6);
				$data["Prices"]["iRide"][$name] = $val;
			}
		}
		foreach ($_POST as $key => $val) {
			if (strpos($key, "iRidePlus_") === 0) {
				$name = substr($key, 10);
				$data["Prices"]["iRidePlus"][$name] = $val;
			}
		}
		foreach ($_POST as $key => $val) {
			if (strpos($key, "iRideLux_") === 0) {
				$name = substr($key, 9);
				$data["Prices"]["iRideLux"][$name] = $val;
			}
		}
		$data["driverShare"] = $_POST["driverShare"];
		$response = $this->settings_model->update_settings($data);
		if ($response) {
