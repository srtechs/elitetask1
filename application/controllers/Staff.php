...
class Staff extends CI_Controller
{
    ...
    {
        $data = $this->staff_model->get_allstaff();

        $this->template->template_render('staff_management', $data);
    }

    public function viewstaff()
    {
        $data['customerlist'] = $this->common->readdatafromcollectionwhereclause("Staff");
        $this->template->template_render('viewstaff', $data);
    }

    public function preferredStaff()
    {
        $data['customerlist'] = $this->common->readdatafromcollectionwhereclause("preferredStaff");
        $this->template->template_render('pstaff_management', $data);
    }

    public function staffDocuments()
    {
        $data['customerlist'] = $this->common->readdatafromcollectionwhereclause("preferredStaff");
        $this->template->template_render('staff_documents', $data);
    }
    ...
    {
        $this->template->template_render('staff_add');
    }
    //editcustomer
    ...
    public function insert_staff()
    {
        ...
        $response = $this->staff_model->add_staff($data);
        if ($response) {
            $this->session->set_flashdata('successmessage', 'New staff added successfully');
            redirect(base_url() . 'staff');
        } else {
            $errormsg = 'Error! Staff already Exists.';
